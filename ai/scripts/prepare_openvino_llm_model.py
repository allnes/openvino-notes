...
EXPORT_MARKER = ".openvino_llm_export_complete"
EXPORT_MANIFEST = "openvino_llm_manifest.json"
...
def export_is_complete(output: Path, model_id: str, weight_format: str) -> bool:
    ...
    if not marker.exists() or not all((output / name).exists() for name in REQUIRED_EXPORT_FILES):
        return False
    if not (output / EXPORT_MANIFEST).exists():
        return False
...
def write_manifest(output: Path, model_id: str, weight_format: str) -> None:
    files = []
    for file in sorted(output.rglob("*")):
        if not file.is_file() or file.name == EXPORT_MANIFEST:
            continue
        files.append(
            {
    ...
    (output / EXPORT_MANIFEST).write_text(json.dumps(manifest, indent=2, sort_keys=True) + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    (args.output / EXPORT_MARKER).write_text(
        f"model_id={args.model_id}\nweight_format={args.weight_format}\n",
        encoding="utf-8",
    )
    write_manifest(args.output, args.model_id, args.weight_format)
